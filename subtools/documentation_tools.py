...
class DocumentationTools:
    ...
    def __init__(self, docs_dir: Path, devdocs_url: str = "http://localhost:9292", logger=None):
            ...
            try:
                ...
                if response.status_code == 200:
                    try:
                        ...
                        return response.text[:5000] + ("..." if len(response.text) > 5000 else "")
                else:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    return json.dumps({
                        ...
                        "all_available": all_docs[:50],  # First 50 to avoid overwhelming
            ...
            try:
                ...
                quick_refs = {
                    "python": {
                        ...
                        "data_types": ["int", "float", "str", "list", "dict", "tuple", "set", "bool"],
                        "common_imports": ["import os", "import sys", "import json", "import requests", "from pathlib import Path"],
                        "useful_functions": ["len()", "range()", "enumerate()", "zip()", "map()", "filter()"]
                    },
                    "javascript": {
                        "syntax": {
                        ...
                        "data_types": ["number", "string", "boolean", "object", "array", "null", "undefined"],
                        "common_methods": ["array.map()", "array.filter()", "array.reduce()", "string.split()", "JSON.parse()"],
                        "es6_features": ["const/let", "arrow functions", "destructuring", "template literals", "modules"]
                    }
                }
                ...
                if language in quick_refs:
                    ...
                    if topic and topic in ref_data:
                        ...
                        return json.dumps({
                            ...
                            "available_topics": list(ref_data.keys()) if isinstance(ref_data, dict) else []
                        }, indent=2)
                else:
    ...
    def register_resources(self, mcp_server):
            ...
            try:
                ...
                for lang, config in self.supported_languages.items():
                    ...
                    languages_info.append({
                        ...
                        "local_docs_available": lang_docs_dir.exists() and any(lang_docs_dir.iterdir()) if lang_docs_dir.exists() else False
                    })
        ...
        async def get_documentation_file(language: str, file_path: str) -> str:
            ...
            try:
                ...
                if not str(full_file_path.resolve()).startswith(str(lang_docs_dir.resolve())):
                ...
                return json.dumps({
                    "language": language,
                    "file_path": file_path,
                    "content": content[:10000] + ("..." if len(content) > 10000 else ""),  # Limit content size
                    "size": len(content),
