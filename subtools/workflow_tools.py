            ...
            try:
                ...
                for service in services:
                    ...
                    if service.get("image"):
                        compose_data["services"][service_name]["image"] = service[
                            "image"
                        ]
                        del compose_data["services"][service_name]["build"]

                if include_volumes:
                    compose_data["volumes"] = {f"{project_name}_data": {}}

                if include_networks:
                    compose_data["networks"] = {
                        f"{project_name}_network": {"driver": "bridge"}
                    }
                ...
                if include_tests:
                    if language == "python":
                        workflow_data["jobs"]["test"]["steps"].append(
                            {"name": "Run tests", "run": "pytest --cov=src tests/"}
                        )
                    elif language == "node":
                        workflow_data["jobs"]["test"]["steps"].append(
