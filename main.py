...
class ServiceConfig:
    ...
    dashboard_enabled: bool = os.getenv("ENABLE_DASHBOARD", "false").lower() == "true"
...
class MCPDockerServer:
    ...
    def _register_web_interface(self):
        ...
        @self.mcp.app.get("/", response_class=HTMLResponse)
        async def status_dashboard():
            """System Status Dashboard (from status_tracking.py)"""
            html_content = """
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <script>
        ...
        @self.mcp.app.get("/api/system", response_class=JSONResponse)
        async def detailed_system_status():
            """API endpoint for detailed system status (compatible with status_tracking.py)"""
            try:
                cpu_percent = psutil.cpu_percent(interval=1)
        ...
        @self.mcp.app.get("/api/nvidia", response_class=JSONResponse)
        async def nvidia_gpu_status():
            """API endpoint for NVIDIA GPU status (compatible with status_tracking.py)"""
            try:
                import subprocess
                result = subprocess.run(['nvidia-smi', '--query-gpu=name,driver_version,temperature.gpu,utilization.gpu,memory.used,memory.total,power.draw', '--format=csv,noheader,nounits'], 
                                      capture_output=True, text=True, timeout=10)
        ...
        @self.mcp.app.get("/api/docker", response_class=JSONResponse)
        async def docker_detailed_status():
            """API endpoint for detailed Docker status (compatible with status_tracking.py)"""
            try:
                import subprocess
